...
def init(train_path, test_path, validation_path, l,toprint):
    ...
    accuracy = accuracy1
    print "Accuracy before pruning:", accuracy1*100
    if(l==0):
        print 'Accuracy after pruning:', accuracy*100
    else:
        ll = random.randint(1,l)
        k = random.randint(1,100)
        dbest = rootNode.postpruning(ll, k, validation_path)
...
if(len(sys.argv))<5:
	print "python id3.py <number of nodes to prune> <path-to-training-set> <path-to-test-set> <path-to-validation-set> <to-print>"
	...
	toprint = int(sys.argv[5])
	init(train_path, test_path, validation_path,l,toprint)
